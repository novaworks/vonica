...
if(!function_exists('nova_woo_get_product_per_page_array')){
    function nova_woo_get_product_per_page_array(){
        $per_page_array = apply_filters('vonica/filter/get_product_per_page_array', Nova_OP::getOption('product_per_page_allow'));
...
function nova_set_cookie_default(){
		if (isset($_GET['per_page']) && $per_page = $_GET['per_page']) {
				add_filter('vonica/filter/get_product_per_page','nova_get_parameter_per_page');
		}
}
...
if(!function_exists('nova_woo_get_product_per_page')){
    function nova_woo_get_product_per_page(){
        return apply_filters('vonica/filter/get_product_per_page', Nova_OP::getOption('shop_product_per_page'));
    }
}
if(!function_exists('nova_woo_get_product_per_row')){
    function nova_woo_get_product_per_row(){
        return apply_filters('vonica/filter/get_product_per_row', Nova_OP::getOption('shop_product_columns'));
    }
}
//==============================================================================
    ...
    function nova_woo_custom_info_tab(){
        ...
        echo '<ul class="nova-product-info-mn">';
				...
				if( 1 == Nova_OP::getOption('single_product_store_availiable') ) {
					...
					echo '<li class="panel_size_guide"><a target="_blank" href="'.esc_url( Nova_OP::getOption('single_product_store_availiable_url') ).'">'.$icon_store.esc_html__( 'Store availability', 'vonica' ).'</a></li>';
				}
				echo '</ul>';
    }
}
			...
			if ( $current_stock > 0 ) {
				echo '<div class="kitify-progress-bar kitify-stock-progress-bar">';
					echo '<div class="stock-info">';
						echo '<div class="total-sold">' . esc_html__( 'Sold:', 'vonica' ) . '<span> ' . esc_html( $total_sold ) . '/'.$total.'</span></div>';
					echo '</div>';
					echo '<div class="progress-area" title="' . esc_html__( 'Sold', 'vonica' ) . ' ' . esc_attr( $percentage ) . '%">';
 		...
 		echo '<div class="options_group">';
 			woocommerce_wp_text_input(
 				array(
 					...
 					'label'       => esc_html__( 'Initial number in stock', 'vonica' ),
 					'desc_tip'    => 'true',
 					'description' => esc_html__( 'Required for stock progress bar option', 'vonica' ),
 					'type'        => 'text',
 				)
 			);
 		echo '</div>';
