...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'vonica' ),
			'archive'   => esc_html__( 'Archives', 'vonica' ),
			'blog'      => esc_html__( 'Blog', 'vonica' ),
			'search'    => esc_html__( 'Search results for', 'vonica' ),
			'not_found' => esc_html__( 'Not Found', 'vonica' ),
			'author'    => esc_html__( 'Author Archives:', 'vonica' ),
			'day'       => esc_html__( 'Daily Archives:', 'vonica' ),
			'month'     => esc_html__( 'Monthly Archives:', 'vonica' ),
			'year'      => esc_html__( 'Yearly Archives:', 'vonica' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'vonica' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'vonica' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'vonica' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
